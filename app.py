...
ARUCO_DATASET_DIR = 'dataset_aruco'  
...
def process_and_save_image(filename):
    ...
    angle_display = cv2.normalize(angle, None, 0, 255,
...
# Processing pipeline for Part 4 (ArUco markers) — fixed
def process_and_save_aruco(filename):
    ...
    # IMPORTANT: always recompute (no early return / caching)
    frame = cv2.imread(input_path)
    if frame is None:
        return None
    ...
    if ids is not None and len(ids) > 0:
        # Draw all detected markers (white boxes + IDs)
        cv2.aruco.drawDetectedMarkers(segmented_image, corners, ids)

        # Flatten: list of (1, 4, 2) -> (N*4, 2)
        all_marker_corners = np.concatenate(
            [c[0] for c in corners], axis=0
        ).astype(np.float32)

        # Only compute hull if we have at least 3 points
        if all_marker_corners.shape[0] >= 3:
            hull = cv2.convexHull(all_marker_corners)  # (H, 1, 2) or (H, 2)

            # Draw a single green polygon that connects the markers
            cv2.polylines(
                segmented_image,
                [hull.astype(int)],
                isClosed=True,
...
@app.route('/aruco')
def index_aruco():
    """Index page for Part 4: lists all ArUco images."""
...
def view_aruco(filename):
    """Results page for Part 4: shows original + segmented ArUco output."""
